...
class PDFCustomizer extends Module
{
	...
	public function install()
	{
	 	if (!parent::install() OR
	 		...
	 		CREATE TABLE '._DB_PREFIX_.'pdfcustomizer (
	 		...
	 		!Db::getInstance()->Execute('
	 		CREATE TABLE '._DB_PREFIX_.'pdfcustomizer_lang (
	...
	public function hookLeftColumn($params)
	{
		...
		$smarty->assign(array(
	 	...
	 	if (!$links)
			return false;
		return $this->display(__FILE__, 'pdfcustomizer.tpl');
	}
	...
	public function getLinks()
	{
	 	...
	 	foreach ($links AS $link)
	 	{
			...
			if (!$texts = Db::getInstance()->ExecuteS('SELECT `id_lang`, `text` FROM '._DB_PREFIX_.'pdfcustomizer_lang WHERE `id_link`='.(int)($link['id_link'])))
				return false;
	...
	{
	 	/* Url registration */
	 	if (!Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'pdfcustomizer VALUES (\'\', \''.pSQL($_POST['url']).'\', '.((isset($_POST['newWindow']) AND $_POST['newWindow']) == 'on' ? 1 : 0).')') OR !$lastId = mysql_insert_id())
	 		return false;
	 	/* Multilingual text */
	 	...
	 	foreach ($languages AS $language)
	 	 	...
	 	 	{
	 	 		if (!Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'pdfcustomizer_lang VALUES ('.(int)($lastId).', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$language['id_lang']]).'\')'))
	 	 			return false;
	 	 	}
	 	 	else
	 	 		if (!Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'pdfcustomizer_lang VALUES ('.(int)($lastId).', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$defaultLanguage]).'\')'))
	 	 			return false;
	 	return true;
	}
	...
	{
	 	/* Url registration */
	 	if (!Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'pdfcustomizer SET `url`=\''.pSQL($_POST['url']).'\', `new_window`='.(isset($_POST['newWindow']) ? 1 : 0).' WHERE `id_link`='.(int)($_POST['id'])))
	 		return false;
	 	/* Multilingual text */
	 	$languages = Language::getLanguages();
	 	$defaultLanguage = (int)(Configuration::get('PS_LANG_DEFAULT'));
	 	if (!$languages)
			 return false;
		if (!Db::getInstance()->Execute('DELETE FROM '._DB_PREFIX_.'pdfcustomizer_lang WHERE `id_link` = '.(int)($_POST['id'])))
			return false ;
	 	foreach ($languages AS $language)
	 	 	if (!empty($_POST['text_'.$language['id_lang']]))
	 	 	{
	 	 		if (!Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'pdfcustomizer_lang VALUES ('.(int)($_POST['id']).', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$language['id_lang']]).'\')'))
	 	 			return false;
	 	 	}
	 	 	else
				if (!Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'pdfcustomizer_lang VALUES ('.(int)($_POST['id']).', '.$language['id_lang'].', \''.pSQL($_POST['text_'.$defaultLanguage]).'\')'))
	 	 			return false;
	 	return true;
	}
	...
	{
	 	return Db::getInstance()->Execute('DELETE FROM '._DB_PREFIX_.'pdfcustomizer WHERE `id_link`='.(int)($_GET['id']));
	}
	...
	public function getContent()
    {
		...
		<script type="text/javascript" src="'.$this->_path.'pdfcustomizer.js"></script>';
